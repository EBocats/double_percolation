...
for frame in range(1,node.source.num_frames):
    node.modifiers.append(ExpressionSelectionModifier(expression = 'DisplacementMagnitude >= 0.6'))
    ...
    try:
        if frame == N:
            export_file(data.tables['clusters'], 'clusters_%s_%s.txt' %(T,frame), 'txt/table', key='clusters')
    except:
        continue
