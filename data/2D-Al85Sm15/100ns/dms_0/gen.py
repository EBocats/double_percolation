...
fpCmd = open('run.bat','w')
strcmd_head='''#!/bin/bash
#SBATCH --job-name=hbyu

'''
# fpCmd.write(strcmd_head)
a=random.randrange(10,100000)

for i in range(200,1010,20):
    fp = open('dms%s.lmp' %i,'w')
    fp.writelines(strlmp %(i))

    fp.close()
    fpCmd.write('mpirun -np 24 lmp_2023 -sf intel -i dms%s.lmp -v R %d\n' %(i,a))
    print(i)
